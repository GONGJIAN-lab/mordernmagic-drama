...
// 2026-09-26: S3 fallback removed — BIG STAR Drama 前提是数据全部走 BytePlus VOD。
// (S3_BUCKET / S3_SIGN_TTL / S3Client 删除；不再有 silent fallback to S3。)
...
// ===== Route: list episodes for a drama — BytePlus only (no S3 fallback) =====
router.get('/episodes/:dramaId', async (req, res) => {
  ...
  if (!vodAdapter) {
    return res.status(500).json({ success: false, error: 'byteplus vod adapter not initialized' });
  }
  ...
  try {
    ...
    const enriched = await Promise.all(episodes.map(async (ep) => {
      ...
      // 2026-09-26: byteplusVid 必须存在，GetPlayInfo 必须成功，否则 500
      if (!ep.byteplusVid) {
        throw new Error('episode ' + ep.episodeNumber + ' has no byteplusVid');
      }
      ...
      if (!bp || !bp.Result || !bp.Result.PlayInfoList || bp.Result.PlayInfoList.length === 0) {
        throw new Error('byteplus getPlayInfo returned no PlayInfoList for ep ' + ep.episodeNumber);
      }
      const playInfo = bp.Result.PlayInfoList[0];
      const mainUrl = playInfo.MainPlayUrl || playInfo.PlayUrl;
      if (!mainUrl) {
        throw new Error('byteplus returned no MainPlayUrl for ep ' + ep.episodeNumber);
      }
